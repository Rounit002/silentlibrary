...
const StaffPayments: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await api.getStaffPayments({
        ...
        toast.error('Failed to load staff payments');
        setPayments([]);
        setStaffList([]);
      } finally {
        setLoading(false);
  ...
  const handleExportCsv = async () => {
    try {
      setIsExporting(true);
      const csvBlob = await api.exportStaffPaymentsCsv({
        branchId: selectedBranchId,
        month: selectedMonth || undefined,
        staffId: selectedStaffId
      });
      ...
      link.href = url;
      link.download = `staff_payments_${suffix}.csv`;
      ...
      console.error('Failed to export staff payments CSV:', error);
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      if (editingPayment) {
        await api.updateStaffPayment(editingPayment.id, payload as any);
        toast.success('Payment updated');
        setEditingPayment(null);
      } else {
        await api.addStaffPayment(payload as any);
  ...
  const handleStaffSubmit = async () => {
    ...
    try {
      if (editingStaff) {
        await api.updateStaff(editingStaff.id, { name: staffFormData.name.trim() });
        toast.success('Staff updated');
        setEditingStaff(null);
      } else {
        await api.addStaff({ name: staffFormData.name.trim() });
  ...
  const handleEditPayment = (payment: StaffPayment) => setEditingPayment(payment);
  ...
  const groupedPayments = payments.reduce((acc, payment) => {
    ...
    return acc;
  }, {} as Record<string, StaffPayment[]>);

  const getBranchName = (p: StaffPayment) =>
    p.branchName ||
